...
- (void)viewDidLoad
{
    ...
    [AppleAccountConnector addObserve:nil isFirstAddCallBack:YES forAppleAccountLoginStateHandler:^(SSDKAppleAccountState state, SSDKUser * _Nonnull user, NSError * _Nonnull error) {
        if (state != SSDKAppleAccountStateAuthorized) {
            [ShareSDK cancelAuthorize:SSDKPlatformTypeAppleAccount result:^(NSError *error) {
                NSString *key = [NSString stringWithFormat:@"%@",@(SSDKPlatformTypeAppleAccount)];
...
- (UIView *)getInfoView:(SSDKPlatformType)platformType
{
    ...
    if([_platforemUserInfos valueForKey:key])
    {
        ...
        if (platformType == SSDKPlatformTypeAppleAccount) {
            textLabel.text = user.rawData[@"fullName"][@"givenName"]?:user.uid;
        }
