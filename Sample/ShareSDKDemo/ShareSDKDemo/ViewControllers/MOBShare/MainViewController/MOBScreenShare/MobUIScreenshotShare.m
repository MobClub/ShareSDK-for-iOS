         ...
         selecetedPlatformType:(MUSSSelecetedPlatformType)selecetedPlatformType
{
    ...
    self.window.windowLevel = [UIApplication currentWindow].windowLevel + 2; //等级需按各APP情况自行设定
    [self.window showOnCurrentScene];
           ...
           selecetedPlatformType:(MUSSSelecetedPlatformType)selecetedPlatformType
{
    ...
    self.window = [[UIWindow alloc] initWithFrame:[UIApplication currentWindow].bounds];
    self.window.backgroundColor = [UIColor clearColor];
...
- (UIButton *)cancelButtonWithFrame:(CGRect)rect
{
    ...
    UIImage *bgImage = [UIImage imageWithColor:[UIColor colorWithWhite:1 alpha:0.5]];
