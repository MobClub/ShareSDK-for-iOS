//
//  MOBTikTokViewController.m
//  ShareSDKDemo
//
//  Created by maxl on 2019/10/28.
//  Copyright © 2019 mob. All rights reserved.
//

#import "MOBTikTokViewController.h"
#import <Photos/Photos.h>

@interface MOBTikTokViewController ()

@end

@implementation MOBTikTokViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    [UIPasteboard generalPasteboard].items = @[];
    platformType = SSDKPlatformTypeTikTokI18N;
    self.title = @"TikTok";
...
//        [assetLocalIds addObject:localId_4];
    ...
    NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
    //product_extra_info
    [parameters SSDKSetupTikTokParamesByAssetLocalIds:assetLocalIds
                                              hashtag:@"ShareSDK"
                                            extraInfo:nil
                                                 type:SSDKContentTypeImage];
    [weakSelf shareWithParameters:parameters];
...
- (void)shareMulVideos
{
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (success) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
                [parameters SSDKSetupTikTokParamesByAssetLocalIds:assetLocalIds hashtag:nil extraInfo:nil type:SSDKContentTypeVideo];
                [weakSelf shareWithParameters:parameters];
            });
        }
    }];
