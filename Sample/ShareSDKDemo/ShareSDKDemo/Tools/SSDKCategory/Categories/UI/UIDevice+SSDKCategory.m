...
@implementation UIDevice (SSDKCategory)
...
- (NSString *)ipAddressCell {
    return [self ipAddressWithIfaName:@"pssdk_ip0"];
}

typedef struct {
    uint64_t en_in;
    uint64_t en_out;
    uint64_t pssdk_ip_in;
    uint64_t pssdk_ip_out;
    uint64_t awdl_in;
    uint64_t awdl_out;
...
static uint64_t _net_counter_get_by_type(net_interface_counter *counter, SSDKNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & SSDKNetworkTrafficTypeWWANSent) bytes += counter->pssdk_ip_out;
    if (type & SSDKNetworkTrafficTypeWWANReceived) bytes += counter->pssdk_ip_in;
    if (type & SSDKNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & SSDKNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & SSDKNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & SSDKNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    } else if ([name hasPrefix:@"pssdk_ip"]) {
                        counter.pssdk_ip_in += counter_in;
                        counter.pssdk_ip_out += counter_out;
                    }
                }
            }
