...
@implementation NSObject (SSDKCategory)
...
#define SSDKPERFORMSELECROR(_args, _return_)\
...
- (nullable id)performSelectorwaitUntilDone:(BOOL)wait withArguments:(SEL)sel onThread:(NSThread *)thread, ...{
    SSDKPERFORMSELECROR(thread, nil)
    if (!wait) [invoctation retainArguments];
...
@implementation NSObject (SwizzleClassMehtod)
...
static const void *ssdkRuntimeDeallocTasks = &ssdkRuntimeDeallocTasks;
static const void *ssdkRuntimeDeallocClassTag = &ssdkRuntimeDeallocClassTag;


static inline void ssdk_swizzleDeallocIfNeed(Class swizzleClass){
    @synchronized (swizzleClass) {
        if (objc_getAssociatedObject(swizzleClass, ssdkRuntimeDeallocClassTag)) return;
        objc_setAssociatedObject(swizzleClass, ssdkRuntimeDeallocClassTag, @1, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)addDeallocTask:(void (^)(id _Nonnull))task{
    @synchronized ([self ssdk_deallocTasks]) {
        [[self ssdk_deallocTasks] addObject:task];
    }
}
