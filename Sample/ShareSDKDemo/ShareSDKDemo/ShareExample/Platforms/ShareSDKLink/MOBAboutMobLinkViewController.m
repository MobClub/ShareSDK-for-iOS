...
- (void)viewDidLoad {
    ...
    [self.navigationBar.middleButton setTitle:@"闭环分享" forState:UIControlStateNormal];
    
    self.navigationBar.rightButton
    .makeChain
    .image(UIImageNamed(@"linkcard_share"), UIControlStateNormal);
    
    
    UIScrollViewModelCreate().delegate(self)
    .addToSuperView(self.view)
    .assignTo(^(__kindof UIView * _Nonnull view) {
        _scrollView = view;
    })
    .addGesture([[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(shareHidden)])
    ...
    .image(UIImageNamed(@"sharelink_ad.jpg"))
    .assignTo(^(__kindof UIView * _Nonnull view) {
        imageV = view;
    })
    .makeMasonry(^(MASConstraintMaker * _Nonnull make) {
        ...
        make.height.equalTo(imageV.mas_width).multipliedBy(size.height/size.width);
    });
    ...
    if (index == 0) {
        [shareParams SSDKSetupShareParamsByText:SHARESDKDEMO_TEXT
                                         images:imageArray//SHARESDKDEMO_IMAGE_LOCALPATH
                                            url:[NSURL URLWithString:SHARESDKDEMO_URL_Header]
                                          title:@"Share SDK"
