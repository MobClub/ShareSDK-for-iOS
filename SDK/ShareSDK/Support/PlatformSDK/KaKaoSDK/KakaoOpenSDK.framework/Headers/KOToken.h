...
/// @abstract accessToken 을 갱신하기 위한 토큰
@property (readonly) NSString *refreshToken;

/// @abstract accessToken 이 만료되는 시각. 네이티브 앱키로 요청한 accessToken 기본 만료시간: 12시간
@property (readonly) NSDate *accessTokenExpiresAt;

/// @abstract refreshToken 이 만료되는 시각. 네이티브 앱키로 요청한 refreshToken 기본 만료시간: 30일.
/// @discussion 이 속성은 1.8.0 버전부터 제공되고 있습니다.<br>
///             만약 앱 업데이트 전에 구버전 SDK로 발급 받았던 토큰이 남아 있어서 [KOSession token]을 통해 접근하는 경우 새로 로그인하거나 refreshToken 이 갱신되기 전까지 값이 없을 수 있습니다.
...
/// @abstract refreshToken 이 사용 가능한 상태인지 여부. 이 값이 YES이면 accessToken 을 계속 갱신하여 사용 가능함.
/// @discussion 클라이언트가 가지고 있는 accessToken 및 refreshToken 의 만료 시간은 서버와 정확히 일치하지 않을 수 있으며
...
/// @return refreshToken 이 있고 현재 시간이 refreshTokenExpiresAt 보다 과거인 경우 YES<br>
///         refreshToken 이 있고 refreshTokenExpiresAt 가 nil인 경우(구버전 SDK에서 발급받은 토큰) YES<br>
///         아니면 NO
- (BOOL)canRefresh;

/// @abstract accessTokenExpiresAt 까지 남은 시간
/// @return 음수이면 accessToken 이 만료되었음을 의미함
