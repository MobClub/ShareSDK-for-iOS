...
/// @abstract 기본 제공되는 템플릿을 이용하여, 카카오톡으로 메시지를 전송합니다.
...
/// @abstract 지정된 URL을 스크랩하여, 카카오톡으로 메시지를 전송합니다.
/// @discussion **제휴를 통해 권한이 부여된 특정 앱에서만 사용 가능합니다.*
...
/// @abstract 지정된 URL을 스크랩하여, 카카오톡의 "나와의 채팅방"으로 메시지를 전송합니다.
/// @param URL 스크랩할 URL. 개발자사이트 앱 설정에 등록된 도메인만 허용됨
/// @param completionHandler 요청 완료시 실행될 block. 오류 처리와 전송 완료 시 수행된다.
...
/// @abstract 미리 지정된 메시지 템플릿(v2)을 사용하여, 카카오톡의 "나와의 채팅방"으로 메시지를 전송합니다.
/// @param templateId 전송할 메시지 템플릿 ID
/// @param templateArgs 메시지 템플릿을 완성하기 위해 필요한 추가 파라미터 정보
/// @param completionHandler 요청 완료시 실행될 block. 오류 처리와 전송 완료 시 수행된다.
